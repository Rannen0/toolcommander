...
def load_model(model_path, tokenizer_path=None, model_type="decoder", **kwargs):
    # 强制离线：先移除可能存在的 local_files_only 键，然后统一设置
    kwargs.pop('local_files_only', None)
    kwargs['local_files_only'] = True

    # 如果传入的是官方模型名，则替换成本地路径（针对检索器）
    if model_path == "facebook/contriever-msmarco":
        local_path = "/home/liuyh/liuyuhang/ToolCommander/models/contriever-msmarco"
        print("✅ 使用本地检索器模型:", local_path)
        model_path = local_path

    # 根据 model_type 选择加载方式
