...
def evaluate_stage1(
    ...
    for idx, attacked_result in enumerate(attacked_results):
        ...
        # 修改：使用当前索引 idx，并检查边界
        if idx < len(train_query_list):
            query_cluster.append(train_query_list[idx])
...
def evaluate_stage2(
    ...
    for idx, attacked_result in enumerate(attacked_results):
        ...
        # 安全地累加 metrics 中的值，如果键不存在则默认为 0
        for key in train_results:
            train_results[key] += data_dict["metrics"].get(key, 0)
        query_cnt += len(dataset[idx])
    print("\tFINISHED")

    # 避免除零错误
    retrieved_pct = 100 * train_results["retrieved"] / query_cnt if query_cnt > 0 else 0
    called_pct = 100 * train_results["called"] / query_cnt if query_cnt > 0 else 0

    print(
        f"{retrieved_pct:.4g}%, {train_results['retrieved']}/{query_cnt}, ",
        end="",
    )
    print(
        f"{called_pct:.4g}%, {train_results['called']}/{query_cnt}, ",
        end="",
    )
    if train_results["dos_total"] != 0:
        print(
            f"{100 * train_results['dos'] / train_results['dos_total']:.4g}%, "
            f"{train_results['dos']}/{train_results['dos_total']}, DOS, ",
            end="",
        )
    if train_results["utc_total"] != 0:
        print(
            f"{100 * train_results['utc'] / train_results['utc_total']:.4g}%, "
            f"{train_results['utc']}/{train_results['utc_total']}, utc, ",
            end="",
        )
    print()
...
def evaluate_direct_stage2(
    ...
    print("[TRAIN]")
    train_retrieved_pct = 100 * train_results["retrieved"] / len(train_query_list) if len(train_query_list) > 0 else 0
    train_called_pct = 100 * train_results["called"] / len(train_query_list) if len(train_query_list) > 0 else 0
    print(f"{train_retrieved_pct:.4g}%, {train_results['retrieved']}/{len(train_query_list)}, ", end="")
    print(f"{train_called_pct:.4g}%, {train_results['called']}/{len(train_query_list)}, ", end="")

    if train_results["dos_total"] != 0:
        print(f"{100 * train_results['dos'] / train_results['dos_total']:.4g}%, "
              f"{train_results['dos']}/{train_results['dos_total']}, DOS, ", end="")
    if train_results["utc_total"] != 0:
        print(f"{100 * train_results['utc'] / train_results['utc_total']:.4g}%, "
              f"{train_results['utc']}/{train_results['utc_total']}, utc, ", end="")
    print()

    print("[EVAL]")
    eval_retrieved_pct = 100 * len(eval_results["retrieved_idx"]) / len(eval_query_list) if len(eval_query_list) > 0 else 0
    eval_called_pct = 100 * len(eval_results["called_idx"]) / len(eval_query_list) if len(eval_query_list) > 0 else 0
    print(f"{eval_retrieved_pct:.4g}%, {len(eval_results['retrieved_idx'])}/{len(eval_query_list)}, ", end="")
    print(f"{eval_called_pct:.4g}%, {len(eval_results['called_idx'])}/{len(eval_query_list)}, ", end="")

    if len(eval_results["dos_total_idx"]) != 0:
        print(f"{100 * len(eval_results['dos_idx']) / len(eval_results['dos_total_idx']):.4g}%, "
              f"{len(eval_results['dos_idx'])}/{len(eval_results['dos_total_idx'])}, DOS, ", end="")
    if len(eval_results["dos_total_idx"]) != 0:
        print(f"{100 * len(eval_results['utc_idx']) / len(eval_results['utc_total_idx']):.4g}%, "
              f"{len(eval_results['utc_idx'])}/{len(eval_results['utc_total_idx'])}, utc, ", end="")
    print()
...
def eval(
    ...
    # 加载攻击结果（如果是文件路径）
    if isinstance(attacked_results, str):
        with open(attacked_results, 'r') as f:
    ...
    if LLM_MODEL is None:
        if target == "tool_llama":
            ...
            # 修正路径：确保与你的实际目录一致
            model_path = "/home/liuyh/liuyuhang/ToolCommander/models/qwen2-7b/qwen/Qwen2-7B-Instruct"

            LLM_TOKENIZER = AutoTokenizer.from_pretrained(
                model_path,
                trust_remote_code=True,
                local_files_only=True,
                padding_side='left'      # 左填充避免生成警告
            )
            # 确保 pad_token 存在
